...
class GridRenderer {
    ...
    _drawRings(ringGrid) {
        ...
        ringGrid.selectAll('circle').data(radialSteps).enter().append('circle').attr('cx', 0).attr('cy', 0).attr('r', rating => this.config.ratingRadialScale(rating)).attr('class', 'ring-grid__circle');
    }
    ...
    _drawRays(rayGrid) {
        ...
        const rayEndpoints = d3.ticks(0, labels.length - 1, labels.length).map(index => {
            const length = ratingRadialScale(maxRating);
            const angle = labelAngularScale(index);
            return Coordinate.polar({ r: length, theta: angle });
        });

        rayGrid.selectAll('line').data(rayEndpoints).enter().append('line').attr('x1', 0).attr('y1', 0).attr('x2', coord => coord.svgX).attr('y2', coord => coord.svgY).attr('class', 'ray-grid__ray');
    }
    ...
    _prerenderLabels(labelGroup) {
        ...
        const labelTexts = labelGroup.selectAll('text').data(labels, label => config.getLabelIndex(label)).enter().append('text').attr('class', 'flavor-labels__label');

        // Then, render each line in a label in its own <tspan>
        labelTexts.selectAll('tspan').data(label => label.split('\n')).enter().append('tspan').attr('class', 'label__line').attr('x', 0).attr('dy', '1em').text(line => line);
    }
}
